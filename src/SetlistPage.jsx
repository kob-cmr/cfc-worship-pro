...
function SectionEditor({ section, onUpdate, onDelete, onMove, isFirst, isLast }) {
  ...
  return (
    <div className="sec-editor-card">
      <div className="sec-editor-header">
        <div className="sec-editor-label">{section.label}</div>
        <div className="sec-editor-actions">
          ...
          <button className="icon-btn danger" onClick={() => onDelete(section.id)} title="Delete section">✕</button>
        </div>
      </div>
      ...
      {tab === "chords" && (
        <textarea className="sec-textarea"
          ...
          placeholder={"Chords only — no lyrics here:\n\nG    D    Em   C\nG    D    G"}
          rows={5}/>
      )}
      {tab === "lyrics" && (
        <textarea className="sec-textarea"
      ...
      {tab === "drums" && (
        <textarea className="sec-textarea"
          ...
          placeholder={"Drum notes for this section...\n\nHi-hat 8ths, kick on 1 & 3, snare on 2 & 4"}
          rows={4}/>
      )}
    </div>
  );
  ...
  return (
    <>
      <style>{`
        ...
        .section-label { display: inline-block; font-weight: 800; font-size: 0.72rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--accent); background: var(--accent-light); padding: 3px 12px; border-radius: 20px; margin-bottom: 8px; border: 1px solid #BFDBFE; }
        ...
        .meta-pill { font-size: 0.7rem; font-weight: 700; background: var(--bg); border: 1px solid var(--border); border-radius: 20px; padding: 2px 9px; color: var(--muted); }

        /* Section editor */
        .sec-editor-list { display: flex; flex-direction: column; gap: 10px; margin-bottom: 4px; max-height: 380px; overflow-y: auto; }
        .sec-editor-list::-webkit-scrollbar { width: 4px; }
        .sec-editor-list::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }
        .sec-editor-card { background: var(--bg); border: 1.5px solid var(--border); border-radius: 10px; overflow: hidden; }
        .sec-editor-header { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; background: var(--panel); border-bottom: 1px solid var(--border); gap: 8px; flex-wrap: wrap; }
        .sec-editor-label { font-size: 0.8rem; font-weight: 800; color: var(--accent); letter-spacing: 0.06em; text-transform: uppercase; }
        .sec-editor-actions { display: flex; gap: 4px; align-items: center; }
        .sec-editor-tabs { display: flex; gap: 2px; padding: 6px 10px 0; background: var(--panel); }
        .sec-tab-btn { padding: 5px 12px; border: none; border-radius: 7px 7px 0 0; font-size: 0.7rem; font-weight: 700; cursor: pointer; background: var(--bg); color: var(--muted); transition: all .15s; border-bottom: 2px solid transparent; }
        .sec-tab-btn.active { background: var(--accent-light); color: var(--accent); border-bottom-color: var(--accent); }
        .sec-textarea { width: 100%; padding: 10px 12px; background: var(--bg); border: none; border-top: 1px solid var(--border); font-family: 'Courier Prime', monospace; font-size: 0.85rem; color: var(--text); resize: vertical; outline: none; min-height: 80px; }
        .sec-visible-btn { padding: 4px 10px; border-radius: 20px; border: 1.5px solid var(--border); background: white; color: var(--green); font-size: 0.68rem; font-weight: 700; cursor: pointer; transition: all .15s; white-space: nowrap; }
        .sec-visible-btn.hidden { color: var(--muted); background: var(--bg); border-style: dashed; }
        .sec-visible-btn:hover { border-color: var(--accent); }

        /* Section builder buttons */
        .section-btn-row { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 4px; }
        .section-add-btn { padding: 7px 12px; border-radius: 20px; border: 1.5px solid var(--border); background: white; color: var(--text2); font-size: 0.72rem; font-weight: 700; cursor: pointer; transition: all .15s; box-shadow: var(--shadow-sm); }
        ...
        .arr-pill { padding: 6px 14px; border-radius: 20px; border: 1.5px solid var(--accent); background: var(--accent-light); color: var(--accent); font-size: 0.72rem; font-weight: 700; cursor: pointer; transition: all .15s; }
        .arr-pill-hidden { border-color: var(--border); background: var(--bg); color: var(--muted); border-style: dashed; text-decoration: line-through; }
        .arr-pill:hover { box-shadow: var(--shadow-md); }

        /* Fullscreen section styles */
        .fs-song-section { margin-bottom: 24px; }
        .fs-section-label { display: inline-block; font-weight: 800; font-size: 0.72rem; letter-spacing: 0.1em; text-transform: uppercase; color: #93C5FD; background: rgba(147,197,253,0.1); padding: 3px 12px; border-radius: 20px; margin-bottom: 10px; border: 1px solid rgba(147,197,253,0.25); }
        ...
        .fs-zoom-btn { padding: 5px 12px; background: #1E293B; border: 1px solid #334155; border-radius: 6px; color: #CBD5E1; font-size: 0.8rem; font-weight: 700; cursor: pointer; min-height: 34px; transition: background .15s; }
        ...
        .dsn-btn { flex: 1; padding: 13px 16px; background: white; border: 1.5px solid var(--border); border-radius: 10px; color: var(--text); font-size: 0.85rem; font-weight: 700; cursor: pointer; transition: all .15s; box-shadow: var(--shadow-sm); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .dsn-btn:hover:not(:disabled) { border-color: var(--accent); color: var(--accent); }
        .dsn-btn:disabled { opacity: .3; cursor: default; }
        .mobile-song-nav { position: fixed; bottom: 0; left: 0; right: 0; display: flex; align-items: stretch; background: white; border-top: 2px solid var(--border); box-shadow: 0 -4px 16px rgba(0,0,0,0.1); min-height: 72px; z-index: 50; }
        ...
        @media (max-width: 767px) {
          .mobile-setlist { display: flex !important; }
